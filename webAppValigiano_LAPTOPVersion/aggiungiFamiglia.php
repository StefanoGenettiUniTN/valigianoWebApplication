...
?>
    ...
    <div class="w3-card w3-margin w3-round">
        <h2 class="w3-margin w3-container" style="text-shadow:1px 1px 0 #444; height: 10%;">Inserimento nuova famiglia</h2>
    </div>

    <div class="w3-card w3-margin w3-round">
        <form class="w3-container w3-card-4 w3-round" action="aggiungiFamiglia.php" method="POST">
            ...
            <a style="float: right; margin-right: 10%;" href="modificaFamiglia.php" class="w3-margin-top">Torna alla lista delle famiglie</a>
...
if(isset($_POST["submit"])){
    ...
    if(!famigliaExists($nome)){
        //Query
        $insertQuery  = "INSERT INTO famiglia (nome) VALUES ( '".$nome."');";
        if(!($ris = $conn->query($insertQuery))){
            echo "<script>alert(\"Errore in fase di inserimento dati.\");</script>";
        }
    }
    else{
        echo "<script>alert(\"Errore, famiglia già presente nel DB\");</script>";
    }
}
