...
?>
    ...
    <div class="w3-card w3-margin w3-round">
        <h2 class="w3-margin w3-container" style="text-shadow:1px 1px 0 #444; height: 10%;">Inserimento nuova societ&agrave</h2>
    </div>

    <div class="w3-card w3-margin w3-round">
        <form class="w3-container w3-card-4 w3-round" action="aggiungiSocieta.php" method="POST">
            ...
            <p style="margin-bottom: 2.5%;">
                <input class="w3-input w3-animate-input" type="text" name="sede" style="width:90%" required>
                <label class="w3-text-teal"><b>Sede</b></label></p>
            <p style="margin-bottom: 2.5%;">
                <input style="float: left;" class="w3-button w3-section w3-teal w3-ripple" type="submit" name="submit" value="REGISTRA"></p>
            <a style="float: right; margin-right: 10%;" href="modificaSocieta.php" class="w3-margin-top">Torna alla lista delle societ&agrave</a>
...
if(isset($_POST["submit"])){
    ...
    if(!societaExists($nome)){
        //Query
        $insertQuery  = "INSERT INTO societa (nome, sede) VALUES ( '".$nome."', '".$sede."');";
        if(!($ris = $conn->query($insertQuery))){
            echo "<script>alert(\"Errore in fase di inserimento dati.\");</script>";
        }
    }
    else{
        echo "<script>alert(\"Errore, società già presente nel DB\");</script>";
    }
}
